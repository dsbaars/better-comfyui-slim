...
if [ ! -d "$COMFYUI_DIR" ] || [ ! -d "$VENV_DIR" ]; then
    ...
    if [ ! -d "$VENV_DIR" ]; then
        ...
        uv pip install --no-cache GitPython numpy pillow opencv-python torchsde sageattention triton insightface  # Common dependencies
    ...
    echo "Installing/updating dependencies for custom nodes..."
    uv pip install --no-cache GitPython numpy pillow sageattention triton insightface # Common dependencies
